...
- 保存与同步节奏固定为：业务持久化状态变化后每 1 秒批量保存 IndexedDB、可同步记录产生实际 diff 后每 5 秒最多成功上传一批到 `POST /hifumi/v1/sync/diff`；除 `413 sync_response_too_large` 缩小批次后的立即重试外，不得连续 POST。远端变更只通过 `GET /hifumi/v1/sync/events` 的 SSE 接收，不得恢复 WebSocket 或周期性 HTTP 空拉取。页面导航、抽屉/Dialog 开关等纯 UI 交互不得触发保存或提前同步。标签页隐藏或离线时必须中止 SSE 与进行中的 POST，同时保留 dirty/outbox；窗口失焦时保持 SSE 连接与当前 `ready` checkpoint，只暂停进行中的 POST，重新聚焦后复用该 SSE 恢复上传。因隐藏或离线关闭 SSE 后，恢复时必须从已持久化 cursor 重连并等待新的 `ready`，才能上传本地 diff。服务端必须保留每用户限流。同步进行中使用 `v-progress-circular`，空闲时显示云端状态图标。
...
- 新同步端点固定为跨源 `POST https://api.luminet.cn/hifumi/v1/sync/diff`、`GET https://api.luminet.cn/hifumi/v1/sync/events?baselineId=<id>&cursor=<n>` 与独立的 `POST https://api.luminet.cn/hifumi/v1/sync/resolve`。POST 使用 JSON + Base64 信封 `{ protobuf }`；SSE 的 `changes`、`ready`、`baseline_mismatch`、`reset_required` 使用 `{ version: 1, protobuf }`，其中 `changes` 的 event ID 必须为 `<baselineId>:<nextCursor>`，控制事件使用 `{ version: 1, code }`。各入口必须独立校验 JSON、Base64 与解码后 Protobuf 的 512 KiB 上限；正常全局 cursor 只由连续 SSE changes 推进。旧 `/api/*` Cloudflare 后端固定返回 `410 Gone`，新浏览器不得回退旧 HTTP 或 WebSocket 协议。
