...
- 日视图必须提供两种可切换的翻页效果：默认保留原有轻量过渡，另提供由本地打包 `turn.js` 驱动的真实 3D Flipbook。动画选择只保存在当前浏览器的 IndexedDB，不进入云同步；Flipbook 使用非交互页面快照承载纸张卷曲与阴影，真实 Vue Day 页始终保持单实例，单次手势只能导航一天。
