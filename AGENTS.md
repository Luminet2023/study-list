...
- 日视图必须提供两种可切换的翻页效果：默认保留原有轻量过渡，另提供由本地打包 `@strata-packages/flipbook` 驱动的 CSS 3D Flipbook。动画选择只保存在当前浏览器的 IndexedDB，不进入云同步；Flipbook 使用非交互页面快照承载纸张卷曲与阴影，真实 Vue Day 页始终保持单实例，单次手势只能导航一天；引擎仅在动画期间工作，不得留下空闲渲染循环。提交目标日期前必须先冻结旧页及两侧日期耳，禁止新日期抢先露出；动画不得恢复插件场景淡入、全页动态 `clip-path` 或 corner curl 重绘，应保持以 GPU 合成的 3D transform 为主。
